...
class SpanConstituencyParserAnalysis(Model):
    ...
    def __init__(self,
        ...
        if evalb_directory_path is not None:
            self._evalb_scorer = EvalbBracketingScorer(evalb_directory_path)
        else:
            self._evalb_scorer = None
        initializer(self)
    ...
    def forward(self,  # type: ignore
        ...
        span_representations = self.span_extractor(elmo_representations, spans, mask, span_mask)
        ...
        if all(batch_gold_trees) and self._evalb_scorer is not None and not self.training:
            ...
            predicted_trees = self.construct_trees(class_probabilities.cpu().data,
                                                   ...
                                                   output_dict["tokens"],
                                                   gold_pos_tags)
            self._evalb_scorer(predicted_trees, batch_gold_trees)

            for f1_metric in self.label_f1.values():
                f1_metric(class_probabilities, span_labels, span_mask)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        if self._evalb_scorer is not None:
            evalb_metrics = self._evalb_scorer.get_metric(reset=reset)
            for metric_name, evalb_metric in evalb_metrics.items():
                all_metrics[metric_name] = evalb_metric

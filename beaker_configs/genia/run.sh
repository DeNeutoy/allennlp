...
export SEED=2
export PYTORCH_SEED=5543
export NUMPY_SEED=5765
...
python scripts/ai2-internal/run_with_beaker.py $CONFIG_PATH/baseline.json --source "glove.1:/glove" --source "genia-ptb:/genia-ptb" \
                                                            --memory 10GB --gpu_count 1 --desc "baseline, glove only" --name "baseline-glove-only-$SEED" \
                                                            --env "SEED=$SEED" --env "PYTORCH_SEED=$PYTORCH_SEED" --env "NUMPY_SEED=$NUMPY_SEED"
# baseline with pubmed glove embeddings
python scripts/ai2-internal/run_with_beaker.py $CONFIG_PATH/baseline_pubmed.json --source "$PUBMED_GLOVE:/glove" --source "genia-ptb:/genia-ptb" \
                                                            --memory 10GB --gpu_count 1 --desc "baseline, bio glove only" --name "baseline-bio-glove-only-$SEED" \
                                                            --env "SEED=$SEED" --env "PYTORCH_SEED=$PYTORCH_SEED" --env "NUMPY_SEED=$NUMPY_SEED"
